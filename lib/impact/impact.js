...
String.prototype.toNumber = function () {
      return parseFloat ( this.replace( /[^\d.-]/g, "" ) ) || 0;
}
...
Array.prototype.findId = function ( id ) {
    if ( !id ) return false; else var r = -1, id = String( id ).toNumber();
    for ( let i = 0; i < this.length; i++ ) {
        if ( this[ i ] instanceof Object && typeof this[ i ][ "id" ] == "number" && this[ i ][ "id" ] == id ) r = i;
    }
...
Math.normalise = function ( val, min, max ) {
    return ( val - min ) / ( max - min );
}

Math.lerp = function ( n, i, x ) {
    return ( x - i ) * n + i;
}

Math.map = function ( v, i, x, j, y ) {
    return Math.lerp( Math.normalise( v, i, x ), j, y );
}
